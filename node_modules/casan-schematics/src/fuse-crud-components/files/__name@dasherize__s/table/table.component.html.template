@if(<%= dasherize(name) %>Resp){
<table mat-table [dataSource]="dataSource" class="mat-elevation-z8">
  <ng-container matColumnDef="id">
    <th mat-header-cell *matHeaderCellDef> ไอดี </th>
...
<div class="flex justify-end bg-white">
  <mat-paginator class="text-md" [length]="<%= dasherize(name) %>Resp.totalItems" [pageSize]="<%= dasherize(name) %>Resp.itemsPerPage"
    [pageIndex]="<%= dasherize(name) %>Resp.currentPage - 1" [pageSizeOptions]="[10, 25, 100]" (page)="onChangePage($event)"
